 ...
 * 
 * Usage: <script src="https://your-cdn.com/apex-header.js"></script>
 */
...
(function() {
  ...
  const CART_SVG = `<svg viewBox="0 0 14 15" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12.0284 4.59541H1.97163C1.82879 4.59252 1.68714 4.6218 1.55715 4.68108C1.42717 4.74036 1.31219 4.82813 1.22073 4.93788C1.12927 5.04763 1.06368 5.17654 1.02881 5.31508C0.993935 5.45362 0.990679 5.59823 1.01928 5.7382L2.06686 10.976C2.11138 11.1943 2.23107 11.3902 2.40508 11.5294C2.57909 11.6686 2.79641 11.7423 3.0192 11.7379H10.9808C11.2036 11.7423 11.4209 11.6686 11.5949 11.5294C11.7689 11.3902 11.8886 11.1943 11.9331 10.976L12.9807 5.7382C13.0093 5.59823 13.0061 5.45362 12.9712 5.31508C12.9363 5.17654 12.8707 5.04763 12.7793 4.93788C12.6878 4.82813 12.5728 4.74036 12.4428 4.68108C12.3129 4.6218 12.1712 4.59252 12.0284 4.59541Z" stroke="#A8A08B" stroke-width="1.25" stroke-linecap="round" stroke-linejoin="round"/><path d="M8.90469 1.26221L10.8094 4.59535" stroke="#A8A08B" stroke-width="1.25" stroke-linecap="round" stroke-linejoin="round"/><path d="M3.19063 4.59535L5.09532 1.26221" stroke="#A8A08B" stroke-width="1.25" stroke-linecap="round" stroke-linejoin="round"/></svg>`;

  function generateCartButton() {
    if (!CONFIG.cart.enabled || !shouldShowCart()) return '';
    return `<button type="button" class="apex-header__cart js-cart" data-action="${CONFIG.cart.action}" aria-label="Cart">${CART_SVG}<span class="apex-header__cart-count js-cart-count">0</span></button>`;
  }
  ...
  function generateNavItems() {
    return CONFIG.nav.map(item => {
      ...
      const submenuHTML = hasSubmenu ? `<div class="apex-header__dropdown">${item.submenu.map(sub => `<a href="${sub.href}" class="apex-header__dropdown-link">${sub.label}</a>`).join('')}</div>` : '';
      return `<li class="apex-header__nav-item${activeClass}"><a href="${item.href}" class="apex-header__nav-link">${item.label}</a>${submenuHTML}</li>`;
    }).join('');
  }
  ...
  function generateMobileNavItems() {
    return CONFIG.nav.map(item => {
      ...
      if (hasSubmenu) {
        return `<li class="apex-mobile-drawer__item${activeClass}"><button type="button" class="apex-mobile-drawer__link js-mobile-accordion">${item.label}<svg class="apex-mobile-drawer__arrow" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 9l6 6 6-6"/></svg></button><div class="apex-mobile-drawer__submenu"><a href="${item.href}" class="apex-mobile-drawer__sublink">${item.label} HOME</a>${item.submenu.map(sub => `<a href="${sub.href}" class="apex-mobile-drawer__sublink">${sub.label}</a>`).join('')}</div></li>`;
      }
      return `<li class="apex-mobile-drawer__item${activeClass}"><a href="${item.href}" class="apex-mobile-drawer__link">${item.label}</a></li>`;
    }).join('');
  }
  ...
  function generateActionLinks(forMobile = false) {
    return CONFIG.actions.map(action => {
      ...
      if (action.type === 'button') {
        return `<button type="button" class="${className}${extraClass}" data-action="${action.action}">${action.label}</button>`;
      }
  ...
  function getHeaderHTML() {
    ...
    return `<header class="apex-header"><div class="apex-header__container"><a href="${desktopLogo.href}" class="apex-header__logo"><img src="${desktopLogo.src}" alt="${desktopLogo.alt}"></a><nav><ul class="apex-header__nav">${generateNavItems()}</ul></nav><div class="apex-header__actions">${generateActionLinks(false)}<button type="button" class="apex-header__mobile-toggle js-mobile-toggle" aria-label="Open menu" aria-expanded="false"><span class="apex-header__hamburger"></span></button>${generateCartButton()}</div></div></header><div class="apex-mobile-backdrop js-mobile-backdrop"></div><div class="apex-mobile-drawer js-mobile-drawer" aria-hidden="true"><div class="apex-mobile-drawer__header"><a href="${mobileLogo.href}" class="apex-mobile-drawer__logo"><img src="${mobileLogo.src}" alt="${mobileLogo.alt}"></a><div class="apex-mobile-drawer__header-actions"><button type="button" class="apex-mobile-drawer__close js-mobile-close" aria-label="Close menu"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M18 6L6 18M6 6l12 12"/></svg></button>${generateCartButton()}</div></div><nav><ul class="apex-mobile-drawer__nav">${generateMobileNavItems()}</ul></nav><div class="apex-mobile-drawer__footer">${generateActionLinks(true)}</div></div><div class="apex-header-spacer"></div>`;
  }
  ...
  class ApexHeader {
    ...
    handleScroll() {
      ...
      if (this.isDrawerOpen) { this.lastScrollY = currentScrollY; return; }
      if (currentScrollY < this.scrollThreshold) { this.showHeader(); this.lastScrollY = currentScrollY; return; }
      if (currentScrollY > this.lastScrollY && currentScrollY > this.scrollThreshold) { this.hideHeaderOnScroll(); }
      else if (currentScrollY < this.lastScrollY) { this.showHeader(); }
      this.lastScrollY = currentScrollY;
    }

    hideHeaderOnScroll() {
      if (!this.isHeaderHidden) { this.isHeaderHidden = true; this.header?.classList.add('is-scrolled-hidden'); }
    }

    showHeader() {
      if (this.isHeaderHidden) { this.isHeaderHidden = false; this.header?.classList.remove('is-scrolled-hidden'); }
    }
    ...
    bindEvents() {
      ...
      window.addEventListener('resize', () => {
        if (window.innerWidth > 989 && this.isDrawerOpen) this.closeDrawer();
      });
    ...
    toggleAccordion(e) {
      ...
      this.accordions.forEach(btn => btn.closest('.apex-mobile-drawer__item')?.classList.remove('is-expanded'));
